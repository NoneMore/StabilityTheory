...
theorem lt_iff_ssubset {p : T.PartialType α} {q : T.PartialType α} :
    (p < q) ↔ (p.toSet ⊂ q.toSet) := by
  constructor <;> intro h
  · exact Set.ssubset_of_ssubset_of_subset h fun a b => b
...
@[simp]
theorem mem_toTheory (φ : L[[α]].Sentence) (p : T.PartialType α) :
    φ ∈ p.toTheory ↔ φ ∈ T.withSet p.toSet := by
  simp [toTheory]
...
theorem toTheory_mono {p : T.PartialType α} {q : T.PartialType α} (h : p ≤ q) :
    p.toTheory ⊆ q.toTheory := by
  simp only [toTheory, withSet, union_subset_iff, subset_union_left, image_subset_iff,
...
theorem not_mem_and_mem_not (p : T.PartialType α) :
    ∀ (φ : L.Formula α), ¬(φ ∈ p ∧ φ.not ∈ p) := by
  rintro φ ⟨hφ, hφnot⟩
  obtain ⟨M⟩ := p.isSatisfiable
  letI : (L.lhomWithConstants α).IsExpansionOn M := LHom.isExpansionOn_reduct _ _
  have hM : φ.Realize fun a ↦ (L.con a : M) := by
    exact (Formula.realize_equivSentence M φ).1 <|
      M.is_model.realize_of_mem _ (p.mem_toTheory_of_mem hφ)
  have hnM : φ.not.Realize fun a ↦ (L.con a : M) := by
    exact (Formula.realize_equivSentence M φ.not).1 <|
      M.is_model.realize_of_mem _ (p.mem_toTheory_of_mem hφnot)
  exact (Formula.realize_not).1 hnM hM
...
theorem coe_liftParams (hAB : A ⊆ B) (p : PartialTypeOver (L := L) A α) :
    ((liftParams hAB p : (L[[B]].completeTheory M).PartialType α) :
      Set (L[[B]].Formula α)) =
      liftSet hAB (p : Set (L[[A]].Formula α)) := by
  rw [liftParams, coe_ofSet]
